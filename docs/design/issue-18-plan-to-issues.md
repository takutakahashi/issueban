...
- Plan を Markdown の checklist として解析できる
...
`#` 行は Plan タイトル候補、`##` 行は Plan 説明、`- [ ]` は Plan item として解析します。チェック済み `- [x]` は既定で Issue 化対象から除外します。item の直後にインデントした行がある場合は、その item の本文として扱います。
