...
Backlog など特定のステータスを issueban 専用にする場合は、ボード設定のカラムで「GitHub Issue を作成しない」を有効にします。このカラムに追加したカードは GitHub Issue を作らず D1 にだけ保存し、issueban 内で移動・削除できます。GitHub Issue を同期するカラムへ移動すると、このとき初めて対応する GitHub Issue を作成します。
