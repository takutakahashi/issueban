...
- GitHub Issue にせず issueban にだけ残すカードの作成
...
Backlog など特定のステータスを issueban 専用にする場合は、ボード設定のカラムで「GitHub Issue を作成しない」を有効にします。このカラムに追加したカードは GitHub Issue を作らず D1 にだけ保存し、issueban 内で移動・削除・説明編集ができます。GitHub Issue を同期するカラムへ移動すると、このとき初めて対応する GitHub Issue を作成します。

カードのコメントバッジから Issue の説明とコメント一覧をモーダルで確認でき、コメントを書き込めます。自分が投稿したコメントはモーダル内で編集できます。コメントは閲覧時に GitHub API から都度取得し、D1 へは保存しません。

カードの Plan アイコンから、Issue をキーにした Plan を登録できます。Plan は `<!-- issueban:plan -->` を先頭に持つ Issue コメントとして保存され、Markdown の `- [ ]` を item として解析します。保存後に item を Issue 化すると、対応する GitHub Issue を作成し、作成済み item は D1 の `plan_item_links` に記録されます。再実行では未作成 item だけを対象にします。
