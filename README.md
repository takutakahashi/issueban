...
カードの Plan アイコンから、見出しと本文を持つ Markdown の実装計画・仕様書を登録できます。チェックリストは任意で、記述した場合だけ明示的な「作業単位」として Issue 化できます。GitHub Issue の Plan は専用コメント、ローカルカードの Plan はワークスペース DB に保存され、どちらも作成済みの作業単位を記録して重複作成を防ぎます。
